<#
.SYNOPSIS
  Запускает сервер заказов при старте Windows (без входа в систему) и перезапускает при сбое.

.EXAMPLE
  lab.bat autostart                             # включить/выключить через меню
...
if (-not (Test-Admin)) { throw "Запустите от администратора (lab.bat поднимает права сам)." }
...
Write-Ok "Автозапуск включён: сервер стартует вместе с Windows."
